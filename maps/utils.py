...
def key_of_min_value(d):
    ...
    # BEGIN Question 0
    "*** REPLACE THIS LINE ***"
    return min([key for key in d if d[key] == min(d[key] for key in d)])
    # END Question 0
...
def enumerate(s, start=0):
    ...
    """
    # BEGIN Question 0
    return zip([x + start for x in range(0, len(s))], s)
    # END Question 0
...
def mean(s):
    ...
    # BEGIN Question 1
    mean = 0
    assert (len(s) >= 1),"The list is empty! Please use a nonempty list"
    for i in s:
        mean += i
    mean = mean / len(s)
    return mean

    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals != nil:
            try:
                ...
                raise SchemeError('number of arguments values does not match with the number of formal parameters')
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        if operands != nil:
            ...
            evaluated_operands = operands
        return scheme_apply(self, evaluated_operands, env)
        # END PROBLEM 5
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        ...
        evaluated_expressions_first = scheme_eval(expressions.first, env)
        if expressions.second == nil and scheme_truep(evaluated_expressions_first):
            return evaluated_expressions_first
        elif scheme_truep(evaluated_expressions_first):
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        return False
    elif not scheme_truep(scheme_eval(expressions.first, env)):
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    def make_formal_pair(bindings):
        if bindings != nil:
            check_form(bindings.first, 2, 2)
            formal_pair = Pair(bindings.first.first, make_formal_pair(bindings.second))
            return formal_pair
        else:
            return nil
    def make_expression_pair(bindings):
        if bindings != nil:
            check_form(bindings.first, 2, 2)
            expression_pair = Pair(scheme_eval(bindings.first.second.first, env), make_expression_pair(bindings.second))
            return expression_pair
        else:
            return nil
    formal_pair = make_formal_pair(bindings)
    expression_pair = make_expression_pair(bindings)
    check_formals(formal_pair)
    child_frame = env.make_child_frame(formal_pair, expression_pair)
    return child_frame
    # END PROBLEM 15
...
class MuProcedure(UserDefinedProcedure):
    ...
    # BEGIN PROBLEM 16
    def make_call_frame(self, args, env):
        new_call_frame = env.make_child_frame(self.formals, args)
        return new_call_frame
    # END PROBLEM 16
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN Extra Credit
            operator = scheme_eval(first, env, True)
            check_procedure(operator)
            return operator.eval_call(expr.second, env)
            # END Extra Credit
    return result
